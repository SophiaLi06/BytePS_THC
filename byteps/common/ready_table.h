...
class ReadyTable {
 public:
  ...
  float GetKeyScale(uint64_t key, int rank);
  void SetKeyScale(uint64_t key, float scale, int rank);
  float GetKeyNorm(uint64_t key, int rank);
  void SetKeyNorm(uint64_t key, float max_norm, int rank);
 ...
 private:
  ...
  std::unordered_map<uint64_t, float[8]> _scale_table;
  // (key, norm) pair, keeps track of the per-worker borm
  std::unordered_map<uint64_t, float[8]> _norm_table;
};
