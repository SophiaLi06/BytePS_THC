...
void byteps_lazy_init() {
  ...
  if (BytePSGlobal::IsCrossPcieSwitch() || BytePSGlobal::IsDistributed()) {
    ...
    func.push_back(ContextCopyLoop);
#endif
    if (BytePSGlobal::IsRootDevice()) {
  ...
  // start the loop functions in func, one thread each loop function
...
Status EnqueueTensor(BPSContext &context, std::shared_ptr<Tensor> input,
                     ...
                     std::shared_ptr<std::vector<QueueType>> queue_list, float norm) {
...
void InitTensor(BPSContext &context, size_t size, int dtype, void *cpubuff) {
  ...
  if (BytePSGlobal::IsCrossPcieSwitch()) {
    ...
    printf("open shared memory for prefix %s key %d\n", name.c_str(), key_list[0]);
  }
...
std::shared_ptr<std::vector<QueueType>> GetContextQueueList(int device) {
...
std::shared_ptr<std::vector<QueueType>> GetAfterContextPushQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();
