...
class _CrossBarrier(_DistributedOptimizer):
    ...
    def __init__(self, model, byteps_opt, num_steps=10**6):
        ...
        for p in model.parameters():
            name = self._parameter_names.get(p.__hash__(), None)
            if name is None:
                name = self._parameter_names.get(p, None)
            print("parameter {} has priority {}".format(name, priority))
    ...
    def _synchronize(self):
        ...
        # for p, (handle, ctx) in sorted(self._handles.items(), key=lambda x: self._priority_indexes[x[0]]):
        ...
        self._sync_time += time.time() - start_time
    ...
    def _push_pull_grad_async(self, p):
        ...
        handle = byteps_push_pull(tensor_compressed, average=True, name="Gradient."+name, \
                                  priority=self._priority_indexes[p])
