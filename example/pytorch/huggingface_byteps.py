...
MODEL_CLASSES = {
    ...
    "dialogpt" : (AutoConfig, AutoModelForSequenceClassification, GPT2Tokenizer),
...
def train(args, train_dataset, model, tokenizer) -> Tuple[int, float]:
    ...
    if args.thc or args.new_inca:
        ...
        compression = hvd.Compression.terngrad(params={'d': pytorch_total_params_trainable, \
            'use_bps_server': args.use_bps_server})
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            throughput_log.append(preds.shape[0] / (time.time()-overall_start))
...
def evaluate(args, model, tokenizer, prefix="", epoch="", iteration="") -> Dict:
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "a") as writer:
            ...
            logger.info("  %s, Accuracy = %s, loss = %s\n", \
                        epoch, str(accuracy), str(eval_loss))
            if epoch != "":
...
def main():
    ...
    if args.model_name_or_path:
        ...
        if model_class is AutoModelForSequenceClassification:
